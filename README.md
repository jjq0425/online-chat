...
| `channelid` | 是   | 字符串 | 聊天频道唯一标识，用于区分不同的聊天房间/频道。若传入时channelId不存在会自动创建该channelId                        |
| `username`  | 否   | 字符串         | 显示在聊天界面的用户名，建议使用 URL 编码（如含空格/特殊字符时）|
| `userid`    | 否   | 字符串    | 用户唯一标识，用于系统识别用户身份，需保证同一用户的 `userid` 唯一    |

> 上述字符串尽量避免存在与http网络地址的特殊标识符冲突的内容，如`/` 、`:`、`&`、`?`
